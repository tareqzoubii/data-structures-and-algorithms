...
const toTitleCase = (arr) => {
  let newArr=arr.map((ele) => ele[0].toUpperCase() + ele.slice(1));
return newArr;
};
...
let biggerThanLuke = (arr) => {
  let x = 77
  return arr.filter(a => {return (parseInt(a.mass)>parseInt(x))?a.name:'' }).map(star=>star.name).join(' - ')
};
...
const detectTicTacToeWin = (board) => {
  let case1 = [board[0][0],board[0][1],board[0][2]]
  let case2 = [board[1][0],board[1][1],board[1][2]]
  let case3 = [board[2][0],board[2][1],board[2][2]]

  let case4 = [board[0][0],board[1][0],board[2][0]]
  let case5 = [board[0][1],board[1][1],board[2][1]]
  let case6 = [board[0][2],board[1][2],board[2][2]]

  let case7 = [board[0][0],board[1][1],board[2][2]]
  let case8 = [board[0][2],board[1][1],board[0][0]]

  let game =[case1,case2,case3,case4,case5,case6,case7,case8]

    for (let x = 0; x < game.length; x++){
        if(game[x][0]==game[x][1] && game[x][0] == game[x][2] && game[x][2]!=''){
            return true
        }
    }
    return false
